...
try {
  session = (await useSession(session_id)).session
} catch (e: any) {
  router.push("/404")
}
...
const join = async () => {
  if (selected_account.value !== $near?.wallet.getAccountId() && pool) {
    ...
    router.push(`${user!.id}/${session_id}`)
  }
}
    ...
    <div v-for="[account_id, character] of Object.entries(characters)" :key="account_id"
      ...
      <div v-if="pool && selected_result">
        ...
        <TokenCard v-for="staked in staked_stakes" :key="staked.slug" class="inline-flex justify-center" :item="stake">
          <input v-model="stake_amount" :placeholder="`${staked.amount}`" type="number" class="flex m-2"
            required="true">
        ...
        <TokenCard v-for="token in registered_stakes" :key="token.slug" :item="token"
