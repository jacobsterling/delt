...
const mint = async (item: Item) => {
  ...
  if (wallet) {
    ...
    const mintResult = await contractRef.awardItem(wallet, item, svg)
...
const removeAttribute = async (item: Item) => {
  const attrKey = "weight"
  await contractRef.connect(wallet.signer)
  await contractRef.removeAttribute(item.tokenId, attrKey)
}
...
const removeStat = async (item: Item) => {
  ...
  await contractRef.connect(wallet.signer)
  await contractRef.removeStat(item.tokenId, attrKey, stat)
}
