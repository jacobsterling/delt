...
import { TokenMetadata, Token, Attr, Stat } from "~~/plugins/near.client"
...
const { $wallet: wallet, $near: near } = useNuxtApp()
// const { data: unminted } = await client.from("unminted").select("*").eq("slug", route.params.slug).single()

const mint = async () => {
  unminted.svg = svg.value
  await near?.mint(unminted)
  // router.push(`/${wallet.profile.username}/${unminted.slug}`)
}

const getTokenMetadata = async (token: Token) => {
  console.log(await near?.deltm.nft_token({
...
<template>
  ...
  <button class="d-button-emerald" @click="getTokenMetadata(unminted)">
    Metadata
  </button>
</template>
