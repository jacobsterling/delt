...
const mint = async () => {
  if (wallet) {
    await contractRef.mint(wallet, unminted, svg.value)
...
<template>
  ...
  <ClientOnly>
    <NuxtLink :href="`https://ropsten.etherscan.io/address/${contractRef.tokenAddress(unminted.upgradable)}`"
      class="d-button-emerald">
      Pointed at contract: {{ contractRef.tokenAddress(unminted.upgradable) }}
    </NuxtLink>
  </ClientOnly>
