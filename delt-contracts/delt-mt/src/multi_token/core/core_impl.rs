...
pub trait MultiTokenMintExisting {
    fn internal_mint_existing(
        &mut self,
...
pub trait MultiTokenInternalBurn {
    fn internal_burn(&mut self, token_owner_id: &AccountId, token_id: &TokenId, amount: &Balance);
}
...
pub struct MultiToken {
    ...
    pub listings: UnorderedMap<TokenId, UnorderedMap<AccountId, Listing>>,
}
...
impl MultiToken {
        ...
        T: IntoStorageKey,
    {
        ...
        Self {
            ...
            next_token_id: 0,
            listings: UnorderedMap::new(StorageKey::ListingsByToken),
        }
    }
    ...
    pub fn internal_mint_with_refund(
        ...
        // Insert new supply
        self.total_supply.insert(&token_id, &owner_amount.unwrap());
        ...
        Token {
            token_id,
            owner_id,
            supply: owner_amount.unwrap_or(0),
            metadata: token_metadata,
        }
    }

    pub fn internal_mint_existing(
        &mut self,
        receiver_id: AccountId,
        token: Token,
        amount: Balance,
        ...
        self.internal_deposit(&token.token_id, &receiver_id, amount);

        if let Some((id, usage)) = initial_storage_usage {
            refund_deposit_to_account(env::storage_usage() - usage, id);
        }

        MultiToken::emit_mint(&receiver_id, &token.token_id, &amount, None);
    ...
    pub fn internal_burn(
        ...
        self.internal_withdraw(&token_id, &account_id, amount.clone());
...
impl MultiTokenCore for MultiToken {
    ...
    fn mt_transfer_call(
        ...
        ext_multi_token_receiver::MultiTokenReceiverExt::mt_on_transfer(
            ext_multi_token_receiver::MultiTokenReceiverExt {
        ...
        .then(ext_self::MultiTokenResolverExt::mt_resolve_transfer(
            ext_self::MultiTokenResolverExt {
                account_id: env::current_account_id(),
                deposit: NO_DEPOSIT,
                static_gas: GAS_FOR_RESOLVE_TRANSFER,
                gas_weight: near_sdk::GasWeight(1),
            },
            vec![old_owner],
            receiver_id,
